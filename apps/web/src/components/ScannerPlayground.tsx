...
import { ShieldAlert, ShieldCheck, Play, Copy, Check, Terminal, Cpu, FileCheck, Code2, AlertTriangle, Layers, Info } from "lucide-react";
...
export const ScannerPlayground: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300">
      ...
      {/* Banner / Title Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl glass-panel border border-cyan-500/20 relative overflow-hidden">
        ...
        <button
          onClick={handleScan}
          disabled={scanning}
          className="self-start md:self-auto px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold text-sm shadow-xl shadow-cyan-500/20 flex items-center gap-2 transition-all transform active:scale-95 disabled:opacity-50"
        >
          {scanning ? (
            <>
          ...
          {PRESET_PAYLOADS.map((preset) => {
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl text-left border transition-all glass-panel-hover flex flex-col justify-between space-y-2 ${
                  isSelected
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          {!scanResult && !scanning && (
            <div className="glass-panel p-12 rounded-2xl border border-slate-800/80 flex flex-col items-center justify-center text-center space-y-3 min-h-[380px]">
          ...
          {scanning && (
            <div className="glass-panel p-12 rounded-2xl border border-slate-800 flex flex-col items-center justify-center text-center space-y-4 min-h-[380px]">
          ...
          {scanResult && (
            <div className="space-y-5 animate-in fade-in duration-300">
                ...
                return (
                  <div className={`p-4 rounded-2xl border flex items-center justify-between ${verdict.bg}`}>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-black/30 hover:bg-black/50 text-xs font-mono font-medium flex items-center gap-1.5 border border-white/10 transition-colors"
                    >
              ...
              {isSimulated && (
                <div className="px-4 py-2 rounded-xl bg-purple-950/30 border border-purple-500/30 text-purple-300 text-xs flex items-center gap-2">
              ...
              <div className="space-y-3">
                ...
                <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-200 flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-400 font-mono text-[11px] flex items-center justify-center font-bold">0</span>
                ...
                <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-200 flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-400 font-mono text-[11px] flex items-center justify-center font-bold">1</span>
                  ...
                  <div className="text-xs text-slate-300 font-mono bg-slate-950/60 p-2.5 rounded-lg border border-slate-800">
                    {scanResult.trust_receipt.verdict.flags.length > 0 ? (
                      <div className="space-y-1">
                ...
                <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-200 flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-400 font-mono text-[11px] flex items-center justify-center font-bold">2</span>
                ...
                <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-200 flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-400 font-mono text-[11px] flex items-center justify-center font-bold">3</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="text-[11px] font-mono text-slate-400 flex items-center justify-between">
                      ...
                      <span className="text-emerald-300 font-bold truncate max-w-[280px]">{scanResult.trust_receipt.signature || "ed25519..."}</span>
                    </div>
                  </div>
                </div>
              </div>

              {/* Raw Signed Receipt JSON Box */}
              <div className="glass-panel p-4 rounded-xl border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
